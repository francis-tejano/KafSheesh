...
function authConfig(
  hop: Pick<JumpHop, 'authType' | 'password' | 'privateKey' | 'passphrase'>,
...
export class SshTunnelService {
  ...
  async openSession(
    ...
    this.logger.log(
      `Opening SSH session for ${clusterId}: ${hops
        .map(
          (hop, index) =>
            `${index}:${hop.username}@${hop.host}:${hop.port}/${hop.authType}`,
        )
        .join(' → ')}`,
    );
    try {
      for (const hop of hops) {
        ...
        this.activity.info(
          'SSH',
          `Connecting ${target} (${hop.authType})`,
          clusterId,
        );
        ...
        this.activity.info(
          'SSH',
          `Ready ${hop.username}@${hop.host}:${hop.port}`,
          clusterId,
        );
        clients.push(client);
        previous = client;
  ...
  async ensureForward(
    ...
    this.logger.log(
      `Forward ${remote} -> 127.0.0.1:${localPort} (${clusterId})`,
    );
    this.activity.info(
      'SSH',
      `Forward ${remote} → 127.0.0.1:${localPort}`,
      clusterId,
    );
    return localPort;
  }
      ...
      const start = (address: string) => {
        client
          ...
          .on('error', (error) => {
            clearTimeout(timeout);
            this.logger.error(
              `SSH error ${hop.username}@${hop.host}:${hop.port}: ${error.message}`,
            );
        ...
        (error, stream) => {
          ...
          client
            ...
            .on('error', (err) => {
              clearTimeout(timeout);
              this.logger.error(
                `SSH error ${hop.username}@${hop.host}:${hop.port} (via jump): ${err.message}`,
              );
              reject(err);
            })
            .connect({
  ...
  private measureLatency(client: Client): Promise<number> {
    return new Promise((resolve) => {
      ...
      const timer = setTimeout(() => {
        this.logger.warn(
          'SSH shell probe timed out; bastion likely allows forwarding only',
        );
        finish();
      }, 1500);
      try {
